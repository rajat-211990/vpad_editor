...
main_application.title('SudoTxt')
...
def exit_func(event = None):
    ...
    try:
        if text_changed:
            mbox = messagebox.askyesnocancel('warning', 'Do you want to save changes?')
            if mbox is True:
                if url:
                    content = text_editor.get(1.0,tk.END)
                    with open(url, 'w', encoding='utf-8') as fw:
...
edit.add_command(label='Copy',image = copy_icon, compound = tk.LEFT,accelerator='Ctrl+N', command = lambda:text_editor.event_generate("<Control c>"))
edit.add_command(label='Paste',image = paste_icon, compound = tk.LEFT,accelerator='Ctrl+O', command = lambda:text_editor.event_generate("<Control v>"))
edit.add_command(label='Cut',image = cut_icon, compound = tk.LEFT,accelerator='Ctrl+S', command = lambda:text_editor.event_generate("<Control x>"))
edit.add_command(label='Clear_All',image = clear_all_icon, compound = tk.LEFT,accelerator='Ctrl+Alt+S', command = lambda:text_editor.delete(1.0,tk.END))
